...
links = spark.read.options(comment="#", delimiter="\t").csv("hdfs://10.10.1.1:9000/input/enwiki-pages-articles/*").toDF("page", "neighbor").repartition("page").cache()
ranks = links.select("page").distinct().withColumn("rank", lit(1))
linkcount = links.groupBy("page").count().cache()
...
ranks.write.option("header",True).csv("hdfs://10.10.1.1:9000/output/pagerank_wiki_4")
