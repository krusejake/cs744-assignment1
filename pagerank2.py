...
links = spark.read.options(comment="#", delimiter="\t").csv("hdfs://10.10.1.1:9000/input/web-BerkStan.txt").toDF("page", "neighbor").repartition(10, "page")
ranks = links.select("page").distinct().withColumn("rank", lit(1)).repartition(10, "page")
linkcount = links.groupBy("page").count().repartition(10, "page")
...
for i in range(0, 10):
    contribs = links.join(linkcount, ["page"], "inner").join(ranks, ["page"], "inner") \
                    ...
                    .groupBy("neighbor").sum("contrib").toDF("page", "contribs").repartition(10, "page")
