...
links = spark.read.options(comment="#", delimiter="\t").csv("hdfs://10.10.1.1:9000/input/enwiki-pages-articles/*").toDF("page", "neighbor")
...
for i in range(0, 10):
    contribs = links.join(linkcount, ["page"], "inner").join(ranks, ["page"], "inner") \
                    ...
                    .groupBy("neighbor").sum("contrib").toDF("page", "contribs") # rename neighbor as page
    ranks = contribs.withColumn("rank", 0.15 + 0.85 * col('contribs')).select("page", "rank")

ranks.write.option("header",True).csv("hdfs://10.10.1.1:9000/output/pagerank_wiki_1")
